...
function OnTriggerStay(obj : Collider)
{
	...
	if(FlagManager.currentFlag == ID && Input.GetKey("e"))
	{
		var i : int;
		...
		Message.GetComponent.<GUIText>().text = "You correctly placed " + FlagManager.CNames[ID] +"!";
		...
		FlagManager.PlacedFlag();
	}
	else
	if(Input.GetKey("e"))
	{
		message = "Current Flag: " + FlagManager.currentFlag + "!= ID: " + ID;
		...
		Message.GetComponent.<GUIText>().text = "Incorrect, that country is " + FlagManager.CNames[ID] +".";
		Destroy(Instantiate(Message),3);
